...
const StatsView = ({ children, className, ...props }: StatsViewProps): React.JSX.Element => {
    ...
    let topArtists = [];
    if (data?.top_artists) {
        let unformattedArtists = data?.top_artists;
        topArtists = unformattedArtists.map((artist, i) => {
            return `#${i + 1} ${artist[0]} `;
        });
    }
    return (
            ...
            <Stack className={classes.container}>
                ...
                {data?.top_artists &&
                    (data?.top_artists).map((artist, i) => {
                        artist = `#${i + 1} ${artist[0]}`;
                        return (
                            <p className={classes.statTextSide}>
                                <span style={{ color: "var(--text-color)" }}>{artist}</span>
                            </p>
                        );
                    })}
            </Stack>

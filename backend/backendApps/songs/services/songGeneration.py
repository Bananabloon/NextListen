...
def parse_openai_json(content: str):
    try:
        ...
        if not match:
            raise ValueError("Nie znaleziono poprawnego formatu JSON w odpowiedzi OpenAI")
...
def generate_songs_with_buffer(prompt, base_prompt, count, buffer_size=10):  # Zwiększono domyślnie do 10
    attempts = 0
    while attempts < 3:
